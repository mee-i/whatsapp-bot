...
class WhatsAppBot {
  constructor() {
    ...
    this.MAX_RECONNECT_ATTEMPTS = parseInt(process.env.MAX_RECONNECT_ATTEMPTS) || 5;
    this.RECONNECT_DELAY = parseInt(process.env.RECONNECT_DELAY) || 5000;
    this.STORE_WRITE_INTERVAL = parseInt(process.env.STORE_WRITE_INTERVAL) || 10000;
      ...
      this.sock = makeWASocket({
        ...
        getMessage: async (key) => {
          try {
            ...
            logger.warn(`Failed to get message ${key.id}:`, error.message);
            return undefined;
          }
        },
  ...
  setupEventHandlers(saveCreds) {
    ...
    this.sock.ev.on('groups.update', async (events) => {
      for (const event of events) {
        try {
          ...
          logger.warn(`Failed to update group metadata for ${event.id}:`, error.message);
        }
      }
    });
    ...
    this.sock.ev.on('group-participants.update', async (event) => {
      try {
        ...
        logger.warn(`Failed to update group participants for ${event.id}:`, error.message);
      }
    });
    ...
    if (shouldReconnect && this.reconnectAttempts < this.MAX_RECONNECT_ATTEMPTS) {
      ...
      setTimeout(async () => {
        try {
          ...
          const backoffDelay = this.RECONNECT_DELAY * Math.pow(2, this.reconnectAttempts - 1);
          setTimeout(() => this.start(), Math.min(backoffDelay, 30000));
        }
